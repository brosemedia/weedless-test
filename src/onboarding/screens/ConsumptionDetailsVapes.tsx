...
export const ConsumptionDetailsVapesScreen: React.FC = () => {
  const { stepNumber, totalSteps, goNext, goBack } = useOnboardingStep('ConsumptionDetailsVapes');
  const profile = useOnboardingStore((state) => state.profile);
  const mergeProfile = useOnboardingStore((state) => state.mergeProfile);
  
  const details = profile.consumptionDetails?.vapes || {};
  const [sessionsPerWeek, setSessionsPerWeek] = useState(details.vapeSessionsPerWeek || 0);
  const [mgTHCPerSession, setMgTHCPerSession] = useState(details.estimatedMgTHCPerSession || 10);
  ...
  const handleSessionsChange = (value: number) => {
    ...
    mergeProfile({
      consumptionDetails: {
        ...
        vapes: { ...details, vapeSessionsPerWeek: value, estimatedMgTHCPerSession: mgTHCPerSession },
      },
    });
  };
  ...
  const handleMgChange = (value: number) => {
    ...
    mergeProfile({
      consumptionDetails: {
        ...profile.consumptionDetails,
        vapes: { ...details, vapeSessionsPerWeek: sessionsPerWeek, estimatedMgTHCPerSession: value },
      },
    });
  };
